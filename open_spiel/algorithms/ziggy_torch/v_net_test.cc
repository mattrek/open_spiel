...
//      VPNetModel::TrainInputs{legal_actions, obs, policy, best_value});
      VPNetModel::TrainInputs{obs, best_value});
  return best_value;
}
...
//  std::vector<Action> legal_actions = state->LegalActions();
  std::vector<float> obs = state->ObservationTensor();
  VPNetModel::InferenceInputs inputs = {obs};
...
void TestModelLearnsSimple(const std::string& nn_model) {
  ...
  while (!state->IsTerminal()) {
    ...
    train_inputs.emplace_back(
        ...
        VPNetModel::TrainInputs{obs, 1});

    VPNetModel::InferenceInputs inputs = {obs}; //{legal_actions, obs};
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    std::cout << absl::StrFormat(
        "%d: Losses(total: %.3f, value: %.3f, l2: %.3f)\n", i,
