...
#ifndef OPEN_SPIEL_ALGORITHMS_ZIGGY_TORCH_VPNET_H_
#define OPEN_SPIEL_ALGORITHMS_ZIGGY_TORCH_VPNET_H_
...
class VPNetModel {
 public:
  ...
  class LossInfo {
   public:
    LossInfo() {}
    LossInfo(double value, double l2)
        : value_(value), l2_(l2), batches_(1) {}
    ...
    double Total() const { return Value() + L2(); }
  ...
  struct InferenceInputs {
    ...
    friend H AbslHashValue(H h, const InferenceInputs& in) {
      return H::combine(std::move(h), in.observations);
    }
  };
  ...
  struct TrainInputs {
    ...
    NOP_STRUCTURE(TrainInputs, observations, value);
  };
...
#endif  // OPEN_SPIEL_ALGORITHMS_ZIGGY_TORCH_VPNET_H_
