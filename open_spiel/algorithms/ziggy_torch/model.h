...
#ifndef OPEN_SPIEL_ALGORITHMS_ZIGGY_TORCH_MODEL_H_
#define OPEN_SPIEL_ALGORITHMS_ZIGGY_TORCH_MODEL_H_
...
class MLPOutputBlockImpl : public torch::nn::Module {
 public:
  MLPOutputBlockImpl(const int nn_width);
  std::vector<torch::Tensor> forward(torch::Tensor x);
...
class ModelImpl : public torch::nn::Module {
 public:
  ...
  std::vector<torch::Tensor> forward(torch::Tensor x);
  std::vector<torch::Tensor> losses(torch::Tensor inputs,
                                    torch::Tensor value_targets);
  void print() const;

 private:
  std::vector<torch::Tensor> forward_(torch::Tensor x);
...
#endif  // OPEN_SPIEL_ALGORITHMS_ZIGGY_TORCH_MODEL_H_
